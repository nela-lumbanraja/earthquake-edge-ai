"""
convert_pnw_earthquake_to_memmap.py
=====================================
Mengonversi dataset PNW kelas 'earthquake' dari format HDF5 (SeisBench)
ke format memmap (.npy) dengan potongan 3s, 5s, dan 10s.

Struktur output mengikuti STEAD/memmap:
    PNW/memmap_earthquake/
...
Catatan PNW vs STEAD:
    - PNW tidak punya kolom back_azimuth_deg & source_distance_km,
      keduanya dihitung dari koordinat source & station (haversine).
    - SNR PNW disimpan sebagai string pipe-separated "E|N|Z" per trace,
      dikonversi ke mean scalar float32.
    - Filter back_azimuth tidak diterapkan (semua trace lolos jika P valid).

Jalankan:
    python convert_pnw_earthquake_to_memmap.py \
        --hdf5  /path/to/PNW/comcat_waveforms.hdf5 \
        --csv   /path/to/PNW/comcat_metadata.csv \
        --out   /path/to/PNW/memmap_earthquake \
...
def extract_window(wave: np.ndarray, p_sample: int,
    ...
    Identik dengan referensi STEAD agar kompatibel saat training.
    """
...
# ─────────────────────────────────────────────────────────────────────────────
# Resume detection (identik dengan referensi)
# ─────────────────────────────────────────────────────────────────────────────
def _detect_resume_point(output_dir: Path, window_specs: list, n: int) -> int:
    ...
    Mengecek memmap terbesar (atau 'full' jika ada).
    """
...
def convert(hdf5_path: str, csv_path: str, output_dir: str,
    ...
    # Filter: hanya kelas earthquake
    df = df[df["source_type"] == "earthquake"].copy().reset_index(drop=True)
    print(f"      Trace earthquake: {len(df):,}")
    ...
    print(f"      Trace dengan P-pick valid: {len(df):,}  (dibuang: {before - len(df):,})")
    ...
    N = len(df)
    if N == 0:
        sys.exit("❌  Tidak ada trace valid!")

    # ── 3. Hitung BAZ & distance (tidak ada di CSV PNW) ───────────────────
    print("[2/5] Menghitung back_azimuth_deg & source_distance_km ...")
    ...
    print("[3/5] Mengalokasi file memmap ...")
    memmaps = {}
    total_gb = 0.0
    for name, n_samp in window_specs:
        ...
        print(f"      waveforms_{name}.npy  shape={shape}  ({mem_gb:.2f} GB)  [{'resume' if mode == 'r+' else 'baru'}]")
    print(f"      Total estimasi ukuran: {total_gb:.2f} GB")
    ...
    if resume:
        ...
        if start_idx > 0:
            print(f"\n      Resume dari trace {start_idx:,} ({start_idx/N*100:.1f}% sudah selesai)")
        else:
            print("\n      Tidak ada progress sebelumnya, mulai dari awal.")

    # ── 6. Baca HDF5 & isi memmap ─────────────────────────────────────────
    if start_idx >= N:
        print("Semua trace sudah dikonversi!")
    else:
        print(f"[4/5] Membaca waveform & mengisi memmap "
        ...
        with h5py.File(hdf5_path, "r") as hf:
            for i in tqdm(range(start_idx, N), desc="Converting",
                ...
                if wave is None:
                    # Biarkan slot ini nol (bisa diidentifikasi saat training)
                    traces_written += 1
                    continue
        ...
        print(f"\n      Selesai: {traces_written:,} trace dalam {elapsed:.0f}s "
              f"({traces_written / elapsed:.0f} traces/s)")

    # ── 7. Simpan trace_names & metadata ─────────────────────────────────
    print("[5/5] Menyimpan trace_names, metadata, dan index.json ...")
    ...
    # P-arrival samples array
    p_arrival = df["trace_P_arrival_sample"].values.astype(np.int32)

    # source_magnitude → gunakan preferred_source_magnitude
    ...
    np.savez(
        ...
        snr_db             = snr_values,
        # Kolom tambahan PNW yang berguna saat training/analisis
    ...
    # ── 8. Simpan index.json (format identik dengan referensi STEAD) ───────
    index = {
        "n_traces"      : N,
        "pre_p_sec"     : pre_p_sec,
        "sampling_rate" : SAMPLING_RATE,
        "source"        : "PNW comcat (earthquake)",
    ...
    for name, n_samp in window_specs:
        ...
        size_gb = round(fpath.stat().st_size / (1024 ** 3), 2) if fpath.exists() else 0
        index["variants"][name] = {
            "file"    : fname,
            "shape"   : [N, N_CHANNELS, n_samp],
            "dtype"   : "float32",
    ...
    for name, n_samp in window_specs:
        ...
        print(f"   waveforms_{name}.npy : shape ({N}, {N_CHANNELS}, {n_samp})  [{size_gb:.2f} GB]")
    ...
    print(f"                        snr_db, source_depth_km, lat/lon, S_arrival")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Konversi PNW earthquake dataset → memmap format (3s/5s/10s)"
    )
    parser.add_argument(
        "--hdf5", required=True,
    ...
    parser.add_argument(
        "--out", required=True,
        help="Folder output, e.g. /path/to/PNW/memmap_earthquake"
    )
    parser.add_argument(
