...
Revisi:
1. Menggunakan LABEL_MAP kanonik 7 kelas.
2. Menggunakan split bersama data/splits_5s.npz.
3. Menghapus augmentasi flip dan roll.
4. Menggunakan augmentasi sah:
   - noise injection
   - non-circular time shift
   - channel dropout horizontal E/N
5. Menggunakan z-score per-trace.
6. Output disimpan ke results/runs/.
7. Fix torch.load PyTorch 2.6 dengan weights_only=False.
8. Menambahkan Dynamic Quantization sebagai teknik kompresi awal.
"""
...
class EqDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # z-score per-trace gabungan 3 channel
        mean = x.mean()
        std = x.std() + 1e-8
        x = (x - mean) / std
...
def main():
    ...
    best_model_path = os.path.join(
        ckpt_dir,
        "best_mobilenetv3_v2.pt"
    )

    print("=" * 60)
    print("MobileNetV3 v2 Training + Dynamic Quantization")
    print("=" * 60)
    print("Device  :", DEVICE)
    print("AMP     :", AMP)
    ...
    quantized_path = os.path.join(
        ckpt_dir,
        "mobilenetv3_v2_dynamic_quantized.pt"
    )
    ...
    plot_path = os.path.join(
        fig_dir,
        "mobilenetv3_v2_baseline_results.png"
    )
    ...
    quant_plot_path = os.path.join(
        fig_dir,
        "mobilenetv3_v2_dynamic_quantized_cm.png"
    )
